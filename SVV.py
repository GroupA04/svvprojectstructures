...
N_x = 81                                         # Number of points in span-wise direction (x-direction) --> should be 100?
q_x, z_cp = q_disc(zcoordinates)                       # This contains the values of q at each point along the span
s, s_acum, s_out = integration(xcoordinates, 0, 1.611, q_x, 100)
individ_integrals = [s]                          # This will have the areas of all the little individual "bars" (will have one less value than N_x)
integral1 = [s_acum]                                  # This will have the accumulated area at each point in x
xcoordinates = xcoordinates              # This will contain all the xcoordinates of our "processed" / "non-raw" data


def cumintegration(N_x , q):
